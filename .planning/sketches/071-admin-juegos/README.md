...
- **ONE LIST** (decision 17) — the reframe that dissolved seven rounds of band negotiation. The page is a single
  ...
  **Exception sections close at rest** (decision 18) — exceptions-first buried the catalog 3,270px down, so
  `Sin datos` and `Borradores` are collapsible and **closed by default**, bringing the catalog to **206px**.
  Anatomy stays one (`1 anatomy` still asserted); the **caret is affordance, not a second kind**, carried only by
  ...
  **The section label carries D-19j's rank, 15/600 full strength** (decision 20) — audited, the ladder had
  inverted: the label was 2px smaller and 2.8x lower contrast than a row inside it, and byte-identical to a row's
