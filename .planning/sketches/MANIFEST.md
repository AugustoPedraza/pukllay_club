...
|---|------|----------------|--------|------|
| 017 | header-composition | With 013/014/015 decided (E / D-refined / A), does the full header row still feel overloaded, is a row-based structure even right, and should the CTA/theme-toggle live in the header at all? Covers mobile+desktop together. | E — toggle relocated to footer, real search-morph interaction (icon converts into input, mobile-verified full-width overlay), CTA scoped to Acerca de only (hero + page-scoped mobile sticky bar); flags reopening 01.1-PATTERNS.md D-05 when implemented | header, composition, search, navigation, mobile, information-architecture, cta-placement, footer |
