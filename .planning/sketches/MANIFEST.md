...
|---|------|----------------|--------|------|
| 017 | header-composition | With 013/014/015 decided (E / D-refined / A), does the full header row still feel overloaded — and if so, what's the real structural alternative? Covers mobile+desktop together. | *(pending review)* | header, composition, search, navigation, mobile, information-architecture |
...
| 015 | active-nav-treatment | `aria-current="page"` is emitted on the current page's nav link but only gets a color change today — too subtle. What's a stronger active-state signal? | *(pending review — Round 2: D = C + subtle underline)* | header, navigation, active-state, accessibility |
| 014 | theme-toggle-weight | The theme toggle is a 3-button segmented control (system/light/dark, 3×44px ≈134px) — the widest single element in the header cluster. Compress it, and if so, how? | *(pending review — Round 2: D strips the card container entirely, bare icons)* | header, theme-toggle, navigation, accessibility |
| 013 | header-action-cluster | Should the "Sumate" CTA be persistent on every page, contextual to landing/About only, or persistent-but-visually-demoted off-landing? | *(pending review — Round 2: D/E shift axis from CTA scope to overall cluster visual weight; CTA scope stays persistent)* | header, cta, navigation, information-architecture |
