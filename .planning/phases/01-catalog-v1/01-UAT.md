...
number: 1
name: Weight-band badge no longer overlaps the title at narrow/mobile widths (G-01-2)
expected: |
  At a narrow/mobile viewport (<640px, 2-column grid), load `/` and confirm the weight-band badge
  (e.g. 'Descubre el hobby') no longer overlaps the game title above it, and that its box visibly
  grows rather than clipping when the label wraps to two lines. Badge text stays entirely inside
  its own tinted box with no visual collision with the title.
