...
const SCROLLS = {
  pagebottom: `window.scrollTo(0, document.documentElement.scrollHeight)`,
  overplain: `(() => { const c = document.querySelector('#contacto') || document.querySelectorAll('.pk-band')[document.querySelectorAll('.pk-band').length-2]; window.scrollTo(0, window.scrollY + c.getBoundingClientRect().bottom - innerHeight + 30); })()`
}
const ARMS = {
  A_status_quo: "",
  E_border_only_plus_clearance: ".pk-about-cta-bar{border-top-color:var(--color-neutral)!important} body:has(.pk-about-cta-bar){padding-bottom:69px!important}"
}
async function main() {
  ...
  try {
    for (const theme of ["light", "dark"]) {
      for (const [arm, css] of Object.entries(ARMS)) {
       for (const [pos, SCROLL] of Object.entries(SCROLLS)) {
        const key = `${theme}-${arm}-${pos}`
        ...
        await client.send("Runtime.evaluate", { expression: SCROLL })
        ...
        await writeFile(join(OUT, `arm2-${key}.png`), Buffer.from(shot.data, "base64"))
       }
      }
    }
    await writeFile(join(OUT, "arms2.json"), JSON.stringify(out, null, 2))
    console.log("done")
  } finally {
