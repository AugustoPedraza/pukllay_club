defmodule PukllayClubWeb.UserAuthTest do
  ...
  @remember_me_cookie "_pukllay_club_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PukllayClubWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})

    %{user: %{staff_fixture() | authenticated_at: DateTime.utc_now(:second)}, conn: conn}
  end
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to the admin dashboard when user is already logged in", %{
      conn: conn,
      user: user
    } do
      conn =
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PukllayClubWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_staff_user/2 (T-01.8.1-02)" do
    test "redirects to / for a scope user whose role is not owner/staff", %{conn: conn} do
      non_staff = %Accounts.User{role: nil}

      conn =
        conn
        |> assign(:current_scope, Scope.for_user(non_staff))
        |> UserAuth.require_staff_user([])

      assert conn.halted
      assert redirected_to(conn) == ~p"/"
    end

    test "passes through for an owner scope", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.require_staff_user([])
  ...
  describe "on_mount :require_staff (T-01.8.1-02)" do
    test "halts and redirects to / for a socket assigned a non-staff scope" do
      non_staff = %Accounts.User{role: nil}

      socket = %LiveView.Socket{
        endpoint: PukllayClubWeb.Endpoint,
        assigns: %{__changed__: %{}, flash: %{}, current_scope: Scope.for_user(non_staff)}
      }

      assert {:halt, _updated_socket} = UserAuth.on_mount(:require_staff, %{}, %{}, socket)
    end

    test "continues for a socket assigned a staff scope", %{user: user} do
      socket = %LiveView.Socket{
        endpoint: PukllayClubWeb.Endpoint,
        assigns: %{__changed__: %{}, flash: %{}, current_scope: Scope.for_user(user)}
      }

      assert {:cont, _updated_socket} = UserAuth.on_mount(:require_staff, %{}, %{}, socket)
    end
  end
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PukllayClubWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
