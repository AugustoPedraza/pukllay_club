defmodule PukllayClubWeb.CoreComponents do
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", "pb-4"]}>
      <div>
        ...
        <p :if={@subtitle != []} class="text-neutral text-sm">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(PukllayClubWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(PukllayClubWeb.Gettext, "errors", msg, opts)
    end
  end
